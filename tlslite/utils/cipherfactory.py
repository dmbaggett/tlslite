...
if cryptomath.tlscryptoLoaded:
    from tlslite.utils import tlscrypto_aes
    from tlslite.utils import tlscrypto_rc4
    from tlslite.utils import tlscrypto_rc2
...
def createRC2(key, IV, implList=None):
    """Create a new RC2 object.

    @type key: str
    @param key: A 5 to 256 byte string.

    @type IV: object
    @param IV: Ignored, whatever it is.

    @rtype: L{tlslite.utils.RC2}
    ...
    if len(IV) != 0:
        raise AssertionError()
    for impl in implList:
        if impl == "tlscrypto" and cryptomath.tlscryptoLoaded:
            return tlscrypto_rc2.new(key)
    raise NotImplementedError()
